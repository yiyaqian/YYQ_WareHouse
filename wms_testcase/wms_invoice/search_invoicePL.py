...
class Search_InvoicePL(unittest.TestCase):
    ...
    def search_invoicePL(self, case_name, IDX, url, methond, content_type, inputNo, tableNo, wallNo, message):
        ...
        if methond == 'POST':
            ...
            re = res.json()  # 转为json格式供assertEqual断言使用
        totalOrder = re['result']['totalOrder']
        logging.info(totalOrder)
        if totalOrder > 0:
            for i in range(0, totalOrder):
                jobNo = re['result']['detailDTOList'][i]['jobNo']
                consignmentNo = re['result']['detailDTOList'][i]['consignmentNo']
                skuId = re['result']['detailDTOList'][i]['skuId']
                WrittenToken.written_jobNoAndskuId(int(IDX)+i, jobNo, skuId, consignmentNo)
        return re
