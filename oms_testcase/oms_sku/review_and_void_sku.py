...
result = get_data('../xls/测试用例数据2.xls', 1)


@ddt.ddt
class ReviewAndVoid_sku(unittest.TestCase):

    def ReviewAndVoid_sku(self, case_name, IDX, url, methond, content_type, id, status, message):
    ...
    @ddt.data(*result)
    @ddt.unpack
    def test_ReviewAndVoid_sku(self, case_name, IDX, url, methond, content_type, id, status, message):
        result = self.ReviewAndVoid_sku(case_name, IDX, url, methond, content_type, id, status, message)
        ids = str(id)
        datas = '{' + '"ids":' + '[' + ids + '],' + '"status":' + '"' + status + '"' + '}'
